...
<div x-show="page === 'sales-history'" class="space-y-2">
    ...
    {{-- Filters --}}
    <div
        class="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-sm flex flex-col md:flex-row gap-4">
        <div class="flex-1 flex flex-col md:flex-row gap-3">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" :placeholder="t('search_sales_placeholder') || t('search_placeholder') || 'Search by Customer Name...'" x-model="salesFilter.search"
                    @input.debounce.300ms="loadSales(); salesPage = 1"
            ...
            <div class="w-full md:w-44 shrink-0">
                ...
                <input type="date" x-model="salesFilter.date" onclick="this.showPicker()" @change="loadSales()"
                ...
                <div class="relative">
                    ...
                    <div x-show="open" x-cloak
                        ...
                        <input type="text" :placeholder="t('search_customer_placeholder')"
                            x-model="salesCustomerSearchQuery" @input.debounce.300ms="searchSalesCustomers()"
                            @click.stop
                            ...
                            <template x-for="cust in salesFilteredCustomers" :key="cust.id">
                                <button type="button" @click="selectSalesCustomer(cust); open = false;"
                                    ...
                                    <span class="text-[10px] text-slate-400 font-mono"
                                        x-text="cust.mobile || 'No Mobile'"></span>
                                </button>
                            </template>

                            <template x-if="salesFilteredCustomers.length === 0">
                                <div class="text-center py-4 text-xs text-slate-400" x-text="t('no_customers_found')">No
                                    customers found.</div>
                            </template>
                        </div>
                    </div>
            ...
            <div class="w-full md:w-44 shrink-0">
                ...
                <select x-model="salesFilter.status" @change="loadSales()"
                    class="block w-full px-3 py-2 border border-slate-300 dark:border-gray-600 rounded-xl text-sm dark:bg-gray-700 dark:text-white">
        ...
        <div class="flex items-end gap-2">
            <button @click="clearSalesFilter()"
                class="px-4 py-2.5 border border-slate-300 dark:border-gray-600 hover:bg-slate-50 dark:hover:bg-gray-700 text-slate-600 dark:text-slate-300 text-sm font-semibold rounded-xl transition-all"
                x-text="t('clear')">Clear</button>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <template
                x-for="sale in (salesLoading ? [] : filteredSales().slice((salesPage - 1) * salesPerPage, salesPage * salesPerPage))"
                :key="sale.id">
                <div
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span
                            :class="sale.status === 'Cancelled' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' : (sale.status === 'Returned' ? 'bg-rose-100 text-rose-800' : (sale.status === 'Partially Returned' ? 'bg-amber-100 text-amber-800' : (sale.status === 'Partially Paid' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' : (sale.status === 'Unpaid' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-emerald-100 text-emerald-800'))))"
                            class="px-2 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap inline-block font-sans"
                            x-text="t(sale.status.toLowerCase().replace(/ /g, '_')) || sale.status"></span>
                    </div>
                    ...
                    <div class="space-y-1 text-[11px] py-1.5 border-y border-slate-100 dark:border-gray-700/50">
                        <div class="flex justify-between">
                            ...
                            <span
                                ...
                                x-text="sale.customer ? sale.customer.name : t('walk_in_customer')"></span>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="text-slate-700 dark:text-slate-300 font-medium font-sans"
                                x-text="t(sale.payment_type.toLowerCase()) || sale.payment_type"></span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-base font-extrabold text-slate-900 dark:text-white font-sans">₹<span
                                    x-text="parseFloat(sale.grand_total).toFixed(2)"></span></span>
                        </div>
                        <template
                            x-if="sale.payment_type === 'Credit' || sale.status === 'Partially Paid' || sale.status === 'Unpaid'">
                            <div
                                ...
                                <span class="text-emerald-600 dark:text-emerald-400 font-semibold"><span
                                        ...
                                        x-text="parseFloat(sale.paid_amount || 0).toFixed(2)"></span></span>
                                <span class="text-rose-600 dark:text-rose-400 font-semibold"><span
                                        x-text="t('due')">Due</span>: ₹<span
                                        x-text="Math.max(0, parseFloat(sale.grand_total) - parseFloat(sale.paid_amount || 0)).toFixed(2)"></span></span>
                            </div>
                        </template>
                    </div>
                    ...
                    <div
                        ...
                        <!-- Edit Button -->
                        <button @click="openEditSaleModal(sale)" :title="t('edit')"
                        ...
                        <!-- Invoice Button -->
                        <button @click="viewInvoice(sale.id)" :title="t('sale_invoice')"
                        ...
                        <!-- Return Button -->
                        <button @click="returnSale(sale.id)" :title="t('return_items')"
                        ...
                        <!-- Cancel Button -->
                        <button @click="openCancelSaleModal(sale)" :title="t('cancel_invoice') || 'Cancel Invoice'"
        ...
        <x-pagination currentPage="salesPage" totalItems="filteredSales().length" perPage="salesPerPage"
                ...
                <div class="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 dark:border-gray-700 bg-slate-50/50 dark:bg-gray-800/80 shrink-0">
                    <div class="flex items-center gap-2.5 text-rose-600 dark:text-rose-400">
                        ...
                        <h3 class="text-base font-bold text-slate-800 dark:text-white"
                            x-text="t('cancel_sale') || 'Cancel Invoice'">Cancel Invoice</h3>
                    </div>
                    <button @click="cancelSaleModalOpen = false"
