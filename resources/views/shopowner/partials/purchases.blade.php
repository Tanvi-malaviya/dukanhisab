...
<div x-show="page === 'purchases'" class="flex flex-col lg:flex-row gap-4 pb-10 lg:pb-0 items-start">
    ...
    <div
        class="flex-1 w-full flex flex-col bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-sm p-4 min-h-[480px] lg:sticky lg:top-0">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
            ...
            <div class="relative">
                <input type="text" id="purchase-barcode" :placeholder="t('scan_barcode_placeholder')"
                    x-model="pos.barcodeInput" @keydown.enter.prevent="handlePurchaseBarcodeScan()"
                ...
                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 4v1m6 11h2m-6 0h-2v4m0-4V8m0 8l-4-4m4 4l4-4"></path>
                    </svg>
                </span>
            </div>
        </div>
                ...
                <div @click="addPurchaseItemById(prod.id)"
                    ...
                    <div>
                        <div class="flex items-start justify-between gap-1.5">
                            <p class="font-bold text-sm text-slate-800 dark:text-white truncate" :title="prod.name"
                                x-text="prod.name"></p>
                            <template x-if="hasPurchaseSupplierCustomPrice(prod)">
                                <span
                                    class="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 shrink-0">Supp</span>
                            </template>
                        </div>
                        <p class="text-[10px] text-slate-400 mt-0.5"><span x-text="t('barcode')">Barcode</span>: <span
                                x-text="prod.barcode"></span></p>
                    </div>
                    <div
                        class="flex justify-between items-end mt-3 pt-2 border-t border-slate-100 dark:border-gray-700/60">
                        <div class="flex flex-col">
                            <template x-if="hasPurchaseSupplierCustomPrice(prod)">
                                <span class="line-through text-[10px] text-slate-400 font-medium">₹<span
                                        x-text="parseFloat(prod.purchase_price).toFixed(2)"></span></span>
                            </template>
                            <span class="text-sm font-extrabold"
                                :class="hasPurchaseSupplierCustomPrice(prod) ? 'text-indigo-600 dark:text-indigo-400' : 'text-primary'">
                                ₹<span x-text="parseFloat(getPurchaseProductPrice(prod)).toFixed(2)"></span>
                            </span>
                        </div>
                        <span
                            class="text-[10px] px-2 py-0.5 rounded-full font-semibold shrink-0 bg-slate-200 text-slate-700"
    ...
    <div
        class="w-full lg:w-96 flex flex-col bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-sm p-4 lg:sticky lg:top-0">
        <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200 mb-3 flex items-center justify-between">
            ...
            <button @click="resetNewPurchase()" class="text-xs text-rose-500 hover:underline"
                x-text="t('clear_all')">Clear All</button>
        </h3>
        ...
        <div class="flex items-center gap-2 mb-4 relative" x-data="{ open: false }" @click.away="open = false">
            <div class="flex-1 relative">
                ...
                <button type="button" @click="open = !open"
                    ...
                    <svg class="w-4 h-4 text-slate-400 transition-transform shrink-0" :class="open ? 'rotate-180' : ''"
                ...
                <div x-show="open" x-cloak
                    ...
                    <input type="text" :placeholder="t('search_supplier_placeholder')"
                        x-model="purchaseSupplierSearchQuery" @input.debounce.300ms="searchPurchaseSuppliers()"
                        @click.stop
                        ...
                        <template x-for="sup in purchaseFilteredSuppliers" :key="sup.id">
                            <button type="button" @click="selectPurchaseSupplier(sup); open = false;"
                                ...
                                <span class="text-[10px] text-slate-400 font-mono"
                                    x-text="sup.mobile || 'No Mobile'"></span>
                            </button>
                        </template>

                        <!-- No suppliers found message -->
                        <template x-if="purchaseFilteredSuppliers.length === 0">
                            <div class="text-center py-4 text-xs text-slate-400" x-text="t('no_suppliers_found')">No
                                suppliers found.</div>
                        </template>
                    </div>
                </div>
            ...
            <button @click="showSupplierModal = true"
                class="p-2 border border-slate-300 dark:border-gray-600 hover:border-primary rounded-xl text-slate-500 hover:text-primary transition-all shrink-0">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="overflow-y-auto space-y-2 pr-1 mb-3 min-h-[140px] max-h-[260px]">
            <template x-for="(item, idx) in newPurchase.items" :key="idx">
                <div
                    class="p-2 bg-slate-50 dark:bg-gray-700/50 rounded-xl border border-slate-100 dark:border-gray-700 flex flex-col gap-1.5">
                    <div class="flex justify-between items-start">
                        <span class="text-xs font-bold text-slate-800 dark:text-white truncate max-w-[180px]"
                    ...
                    <div class="flex justify-between items-center gap-1.5">
                        <div
                            class="flex items-center border border-slate-200 dark:border-gray-600 rounded-lg overflow-hidden bg-white dark:bg-gray-800">
                            <button @click="if (item.quantity > 1) item.quantity--"
                                class="px-2 py-0.5 bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-slate-300">-</button>
                        ...
                        <div class="flex items-center gap-0.5">
                            ...
                            <input type="number" step="0.01" x-model.number="item.purchase_price"
                        ...
                        <div class="flex items-center gap-0.5">
                            ...
                            <input type="number" step="0.01" x-model.number="item.discount" placeholder="0"
                                class="w-12 px-1 py-0.5 border border-amber-300 dark:border-amber-600 rounded text-center text-xs dark:bg-gray-700 dark:text-white text-amber-600 font-semibold">
                        </div>
                        <span class="text-xs font-bold text-slate-700 dark:text-slate-300">₹<span
                                x-text="Math.max(0, (item.purchase_price * item.quantity) - (parseFloat(item.discount) || 0)).toFixed(2)"></span></span>
                    </div>
                </div>
            </template>
            <template x-if="newPurchase.items.length === 0">
                <div class="text-center py-10 text-slate-400 text-sm" x-text="t('purchase_cart_empty')">Cart is empty.
                    Click products or scan barcodes to begin.</div>
            </template>
        </div>
        ...
        <div class="border-t border-slate-200 dark:border-gray-700 pt-3 space-y-2 shrink-0">
            ...
            <div
                class="flex justify-between text-sm font-bold border-t border-dashed border-slate-200 dark:border-gray-700 pt-1.5">
            ...
            <div class="pt-1">
                <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1"
                    x-text="t('payment_type')">Payment Type</label>
                <div class="grid grid-cols-4 gap-1.5">
                    <button @click="setPurchasePaymentType('Cash')"
                        :class="newPurchase.payment_type === 'Cash' ? 'bg-primary text-white' : 'bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-slate-300'"
                        class="py-2 text-center text-xs font-bold rounded-lg transition-all"
                        x-text="t('cash')">Cash</button>
                    <button @click="setPurchasePaymentType('UPI')"
                        :class="newPurchase.payment_type === 'UPI'  ? 'bg-primary text-white' : 'bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-slate-300'"
                        class="py-2 text-center text-xs font-bold rounded-lg transition-all"
                        x-text="t('upi')">UPI</button>
                    <button @click="setPurchasePaymentType('Bank')"
                        :class="newPurchase.payment_type === 'Bank' ? 'bg-primary text-white' : 'bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-slate-300'"
                        class="py-2 text-center text-xs font-bold rounded-lg transition-all"
                        x-text="t('bank')">Bank</button>
                    <button @click="setPurchasePaymentType('Credit')"
                        :class="newPurchase.payment_type === 'Credit' ? 'bg-primary text-white' : 'bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-slate-300'"
                        class="py-2 text-center text-xs font-bold rounded-lg transition-all"
                        x-text="t('credit')">Credit</button>
                </div>
            </div>
            ...
            <div class="pt-1">
                <div class="flex justify-between items-center mb-1">
                    <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">Paid Amount (ચૂકવેલ
                        રકમ):</label>
                    <span class="text-[10px] text-slate-400 font-mono">Max: ₹<span
                            x-text="calculatePurchaseTotal().toFixed(2)"></span></span>
                </div>
                <div class="relative">
                    <span
                        class="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400 text-xs font-bold">₹</span>
                    <input type="number" step="0.01" x-model.number="newPurchase.paid_amount" placeholder="0.00"
                ...
                <div class="flex justify-between items-center text-xs mt-1.5 px-1 font-semibold"
                    x-show="getPurchaseDueAmount() > 0">
                    <span class="text-emerald-600">Paid: ₹<span
                            x-text="getPurchasePaidAmount().toFixed(2)"></span></span>
                    <span class="text-rose-600">Payable Due: ₹<span
                            x-text="getPurchaseDueAmount().toFixed(2)"></span></span>
                </div>
            </div>
...
<div x-show="page === 'purchase-history'" class="space-y-2">
    ...
    <div
        class="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-sm flex flex-wrap gap-3 items-end">
        {{-- Search Bar --}}
            ...
            <div class="relative">
                ...
                <div x-show="open" x-cloak
                    ...
                    <input type="text" :placeholder="t('search_supplier_placeholder')"
                        ...
                        @input.debounce.300ms="searchPurchaseHistorySuppliers()" @click.stop
                        class="block w-full px-3 py-1.5 border border-slate-200 dark:border-gray-700 rounded-lg text-xs dark:bg-gray-900 dark:text-white bg-slate-50 focus:outline-none focus:border-primary">
        ...
        <div class="flex gap-2 w-full sm:w-auto">
            <button @click="clearPurchaseFilter()"
                class="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-slate-600 dark:text-slate-300 text-sm font-semibold rounded-xl transition-all cursor-pointer"
                x-text="t('clear')">Reset</button>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        <template x-if="purchasesLoading">
            <div
                class="bg-white dark:bg-gray-800 p-8 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-sm text-center">
                <div
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            <template x-for="pur in (purchasesLoading ? [] : filteredPurchases())" :key="pur.id">
                <div
                    class="bg-white dark:bg-gray-800 rounded-xl border border-slate-200 dark:border-gray-700 shadow-sm p-3.5 hover:shadow-md transition-all flex flex-col justify-between space-y-3">
                    {{-- Card Header --}}
                    <div class="flex justify-between items-center">
                        ...
                        <span
                            :class="pur.status === 'Cancelled' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' : (pur.status === 'Returned' ? 'bg-rose-100 text-rose-800' : (pur.status === 'Partially Returned' ? 'bg-amber-100 text-amber-800' : (pur.status === 'Partially Paid' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' : (pur.status === 'Unpaid' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-emerald-100 text-emerald-800'))))"
                            class="px-2 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap inline-block font-sans"
                            x-text="t(pur.status ? pur.status.toLowerCase().replace(/ /g, '_') : 'completed') || pur.status"></span>
                    </div>
                    ...
                    <div class="space-y-1 text-[11px] py-1.5 border-y border-slate-100 dark:border-gray-700/50">
                        <div class="flex justify-between">
                            ...
                            <span
                                class="text-slate-700 dark:text-slate-300 font-semibold font-sans truncate max-w-[150px]"
                                x-text="pur.supplier ? pur.supplier.name : t('walk_in_supplier')"></span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-400 font-sans" x-text="t('date') + ':'">Date:</span>
                            <span class="text-slate-700 dark:text-slate-300 font-sans"
                                x-text="formatDate(pur.purchase_date)"></span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-400 font-sans" x-text="t('payment_type') + ':'">Payment:</span>
                            <span class="text-slate-700 dark:text-slate-300 font-medium font-sans"
                                x-text="t(pur.payment_type.toLowerCase()) || pur.payment_type"></span>
                        </div>
                    </div>

                    <template x-if="pur.status === 'Cancelled' && pur.cancellation_reason">
                        <div
                            class="text-[10px] text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 p-1.5 rounded-lg border border-red-200 dark:border-red-800/40">
                    ...
                    <div class="space-y-1">
                        <div class="flex justify-between items-center">
                            <span class="text-xs font-semibold text-slate-500 font-sans"
                                x-text="t('total') + ':'">Total:</span>
                            <span class="text-base font-extrabold text-slate-900 dark:text-white font-sans">₹<span
                        ...
                        <template
                            ...
                            <div
                                ...
                                <span class="text-emerald-600 dark:text-emerald-400 font-semibold"><span
                                        x-text="t('paid')">Paid</span>: ₹<span
                                        x-text="parseFloat(pur.paid_amount || 0).toFixed(2)"></span></span>
                                <span class="text-rose-600 dark:text-rose-400 font-semibold"><span
                                        x-text="t('due')">Due</span>: ₹<span
                                        x-text="Math.max(0, parseFloat(pur.total_amount) - parseFloat(pur.paid_amount || 0)).toFixed(2)"></span></span>
                            </div>
                        </template>
                    </div>
                    ...
                    <div
                        ...
                        <!-- Edit Button -->
                        <button @click="openEditPurchaseModal(pur)" :title="t('edit')"
                        ...
                        <!-- Invoice Button -->
                        <button @click="viewPurchase(pur.id)" :title="t('purchase_invoice')"
                        ...
                        <!-- Return Button -->
                        <button @click="returnPurchase(pur.id)" :title="t('return_purchase_items')"
                        ...
                        <!-- Cancel Button -->
                        <button @click="openCancelPurchaseModal(pur)" :title="t('cancel_purchase') || 'Cancel Purchase'"
        ...
        <x-pagination currentPage="purchasesPage" totalItems="purchasesTotal" perPage="purchasesPerPage"
    ...
    <template x-teleport="body">
        <div x-show="cancelPurchaseModalOpen" x-cloak
            class="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
            <div @click.away="cancelPurchaseModalOpen = false"
                class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md my-auto overflow-hidden flex flex-col max-h-[calc(100dvh-2rem)] border border-slate-200 dark:border-gray-700">
                <div class="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 dark:border-gray-700 bg-slate-50/50 dark:bg-gray-800/80 shrink-0">
                    <div class="flex items-center gap-2.5 text-rose-600 dark:text-rose-400">
                        <div class="p-1.5 bg-rose-50 dark:bg-rose-900/30 rounded-lg">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <h3 class="text-base font-bold text-slate-800 dark:text-white"
                            x-text="t('cancel_purchase') || 'Cancel Purchase'">Cancel Purchase</h3>
                    </div>
                    <button @click="cancelPurchaseModalOpen = false"
                        class="p-1 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-gray-700 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="p-5 space-y-3.5 overflow-y-auto flex-1">
                        <div class="p-3 bg-slate-50 dark:bg-gray-700/50 rounded-xl space-y-1.5 text-xs border border-slate-100 dark:border-gray-700/60">
                            <div class="flex justify-between font-semibold">
                                <span class="text-slate-500 dark:text-slate-400" x-text="(t('purchase_bill') || 'Purchase Bill') + ':'">Purchase Bill:</span>
                                <span class="text-primary font-mono font-bold" x-text="purchaseToCancel.purchase_number"></span>
                            </div>
                            <div class="flex justify-between font-semibold">
                                ...
                                <span class="text-slate-800 dark:text-white font-bold">₹<span
                                        x-text="parseFloat(purchaseToCancel.total_amount).toFixed(2)"></span></span>
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <span x-text="t('cancel_purchase_reversal_notice') || 'Cancelling will roll back inventory added by this bill, reverse supplier dues/cash paid, post a CashBook reversal entry, and keep this record marked as Cancelled.'"></span>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea x-model="cancelPurchaseReason" rows="2"
                                :placeholder="t('cancel_purchase_reason_placeholder') || 'Enter the reason for cancelling this purchase bill...'"
                        ...
                        <div class="flex gap-2.5 pt-1">
                            <button type="button" @click="cancelPurchaseModalOpen = false"
                                class="flex-1 py-2.5 bg-slate-100 dark:bg-gray-700 hover:bg-slate-200 dark:hover:bg-gray-600 text-slate-700 dark:text-slate-200 text-xs font-semibold rounded-xl transition-all cursor-pointer"
                                x-text="t('close')">Close</button>
                            <button type="button" @click="submitCancelPurchase()"
                                ...
                                x-text="t('confirm_cancellation') || 'Confirm Cancellation'">
