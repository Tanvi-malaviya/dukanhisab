    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <template x-if="bankAccountsLoading && bankAccounts.length === 0">
            <div class="bg-slate-50 dark:bg-gray-800/40 relative overflow-hidden p-4 rounded-3xl border border-slate-200 dark:border-gray-700 flex flex-col justify-center items-center min-h-[160px] shadow-sm">
                ...
                <p class="text-xs text-slate-400 mt-2 font-medium" x-text="t('loading_bank_details')">Loading bank account details...</p>
            </div>
        </template>
        ...
        <template x-if="!bankAccountsLoading && bankAccounts.length === 0">
            <div class="bg-slate-50 dark:bg-gray-800/40 relative overflow-hidden p-4 rounded-3xl border border-slate-200 dark:border-gray-700 flex flex-col justify-center items-center min-h-[160px] shadow-sm">
                ...
                <p class="text-xs text-slate-400 font-medium" x-text="t('no_bank_linked')">No bank accounts linked.</p>
            </div>
        </template>
            ...
            <div class="bg-primary relative overflow-hidden p-4 text-white rounded-3xl shadow-lg border border-teal-700/30 flex flex-col justify-between min-h-[160px]"
                ...
                <div class="z-10">
                    <p class="text-[10px] uppercase tracking-wider font-semibold"
                        style="color: rgba(255,255,255,0.85) !important;" x-text="t('available_bank_balance')">Available Bank Balance</p>
                    <div class="flex items-baseline gap-2 mt-0.5">
                        ...
                        <template x-if="parseFloat(acc.balance) < 0">
                            <span class="px-1.5 py-0.5 bg-rose-500/30 text-[10px] font-bold rounded border"
                                style="color: #FECACA !important; border-color: rgba(239, 68, 68, 0.4) !important;" x-text="t('overdrawn')">Overdrawn</span>
                        </template>
                    </div>
                </div>
            </div>
        ...
        <div class="p-4 bg-slate-50 dark:bg-gray-800/40 rounded-3xl border border-slate-200 dark:border-gray-700 shadow-sm flex flex-col justify-between min-h-[160px]">
            <div class="flex items-center justify-between mb-2">
                <h4 class="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-2">
                    ...
                    <span x-text="t('quick_actions')">Quick Actions</span>
                </h4>
            </div>
            <div class="grid grid-cols-3 gap-2 flex-grow">
                ...
                <button
                    ...
                    <span class="text-[10px] font-bold text-emerald-700 dark:text-emerald-300 text-center leading-tight" x-text="t('deposit_cash')">Deposit Cash</span>
                </button>
                ...
                <button
                    ...
                    <span class="text-[10px] font-bold text-rose-700 dark:text-rose-300 text-center leading-tight" x-text="t('withdraw_cash')">Withdraw Cash</span>
                </button>
                ...
                <button
                    ...
                    <span class="text-[10px] font-bold text-blue-700 dark:text-blue-300 text-center leading-tight" x-text="t('reconciliation_info')">Reconciliation Info</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <div
        ...
        <div
            ...
            <h4 class="font-bold text-slate-800 dark:text-white text-sm flex items-center gap-2">
                ...
                <span x-text="t('recent_bank_upi_activity')">Recent Bank &amp; UPI Activity</span>
            </h4>
            <span class="text-xs text-slate-400" x-text="getBankTransactions().length + ' ' + (t('total_records') || 'Total records')"></span>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200 dark:divide-gray-700">
                <thead>
                    <tr class="bg-slate-50 dark:bg-gray-700/30">
                        <th class="px-6 py-3 text-left text-xs font-bold text-slate-400 uppercase tracking-wider" x-text="t('date')">Date</th>
                        <th class="px-6 py-3 text-left text-xs font-bold text-slate-400 uppercase tracking-wider" x-text="t('description')">Description</th>
                        <th class="px-6 py-3 text-left text-xs font-bold text-slate-400 uppercase tracking-wider" x-text="t('method')">Method</th>
                        <th class="px-6 py-3 text-left text-xs font-bold text-slate-400 uppercase tracking-wider" x-text="t('type')">Type</th>
                        <th class="px-6 py-3 text-right text-xs font-bold text-slate-400 uppercase tracking-wider" x-text="t('amount')">Amount</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-gray-700">
                    ...
                    <template x-for="tr in (cashbookLoading ? [] : getBankTransactions().slice((bankPage - 1) * bankPerPage, bankPage * bankPerPage))" :key="tr.id">
                        <tr class="hover:bg-slate-50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <td class="px-6 py-3.5 text-xs font-bold text-slate-500 uppercase tracking-wide"
                                x-text="t(tr.payment_method ? tr.payment_method.toLowerCase() : '') || tr.payment_method"></td>
                            <td class="px-6 py-3.5 text-xs">
                                <span
                                    ...
                                    x-text="tr.type === 'cash_in' ? t('deposit_in') : t('withdrawal_out')"></span>
                            </td>
                    ...
                    <template x-if="!cashbookLoading && getBankTransactions().length === 0">
                        <tr>
                            <td colspan="5" class="text-center text-slate-400 py-10 text-sm" x-text="t('no_recent_bank_upi')">
            ...
            <div
                ...
                <div
                    ...
                    <h3 class="font-bold text-slate-800 dark:text-white flex items-center gap-2">
                        ...
                        <span
                            x-text="transferForm.type === 'deposit' ? t('deposit_cash_to_bank') : t('withdraw_cash_from_bank')"></span>
                    </h3>
                ...
                <form @submit.prevent="submitBankTransfer(); showTransferModal = false" class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1" x-text="t('transfer_remarks')">Transfer Remarks / Description</label>
                        <input type="text" required :placeholder="t('notes') + '...'" x-model="transferForm.description"
                            class="block w-full px-3 py-2.5 border border-slate-300 dark:border-gray-600 rounded-xl text-sm dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary">
                    </div>
                    <button type="submit"
                        class="w-full py-3 bg-primary hover:bg-primary-hover text-white text-sm font-semibold rounded-xl shadow-md transition-all" x-text="t('submit_transfer')">Submit Transfer</button>
                </form>
            </div>
        </div>
    </template>
    ...
    <template x-teleport="body">
        <div x-show="showReconciliationModal" x-cloak
            ...
            <div
                ...
                @click.outside="showReconciliationModal = false">
                <div
                    class="px-6 py-4 border-b border-slate-200 dark:border-gray-700 flex justify-between items-center bg-slate-50 dark:bg-gray-700/20">
                    <h3 class="font-bold text-slate-800 dark:text-white flex items-center gap-2">
                        ...
                        <span x-text="t('automatic_reconciliation')">Automatic Reconciliation</span>
                    </h3>
                ...
                <div class="p-6 space-y-4">
                    <div class="p-4 bg-blue-50/50 dark:bg-blue-950/10 rounded-2xl border border-blue-100/50 dark:border-blue-900/30">
                        <p class="text-sm text-slate-600 dark:text-slate-300 leading-relaxed" x-text="t('reconciliation_desc_1')">
                            Bank balances are automatically updated and aggregated from Cash Book transactions marked with payment method 'bank' or 'upi' (such as Sales, Sales Returns, Purchases, or Expenses).
                        </p>
                    </div>
                    <div class="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-gray-700">
                        <p class="text-sm text-slate-600 dark:text-slate-300 leading-relaxed" x-text="t('reconciliation_desc_2')">
                            Use the Deposit and Withdraw buttons to shift funds between physical cash-in-hand and your bank account.
                        </p>
                    </div>
                    <button @click="showReconciliationModal = false"
                        class="w-full py-3 bg-primary hover:bg-primary-hover text-white text-sm font-semibold rounded-xl shadow-md transition-all" x-text="t('close')">
                        Close
                    </button>
                </div>
            </div>
